...
public interface MinecraftCommand<S extends MinecraftSource<P, C, M>, P, C, M, D extends MinecraftDesign<M>, L extends MinecraftLiteralBuilder<?, ?, S, P, C, M>, A extends MinecraftArgumentBuilder<?, ?, ?, S, P, C, M>> extends Command<S, M, D, L, A> {
    ...
    default void runSafe(List<String> input, S source) {
        try {
            ...
            if (e instanceof EndOfCommandParseException ||
                ...
                List<List<Node<S, M>>> branches = this.flatten(source);
                source.sendMessage(this.getDesign().getHelpMessage(this, branches));
                return;
            }
